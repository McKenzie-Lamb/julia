...
size_t jl_static_show_x(JL_STREAM *out, jl_value_t *v, int depth)
{
    ...
    if (v == NULL) {
        ...
        n += jl_printf(out, "%lld", (long long)jl_unbox_int64(v));
    }
        ...
        n += jl_printf(out, "0x%016llx", (long long)jl_unbox_uint64(v));
    }
...
#ifdef _P64
        n += jl_printf(out, "0x%016llx",
                       (long long)(int64_t)jl_unbox_voidpointer(v));
#else
        n += jl_printf(out, "0x%08x", (int)jl_unbox_voidpointer(v));
#endif
    }
        ...
        n += jl_printf(out, "GenSym(%d)", (int)((jl_gensym_t*)v)->id);
    }
        ...
        n += jl_printf(out, "%d:", (int)jl_labelnode_label(v));
    }
    else if (jl_is_gotonode(v)) {
        n += jl_printf(out, "goto %d", (int)jl_gotonode_label(v));
    }
        ...
        n += jl_printf(out, "# line %d", (int)jl_linenode_line(v));
    }
